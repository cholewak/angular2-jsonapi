...
export class JsonApiDatastore {
    ...
    if (this.globalHeaders) {
      this.globalHeaders.keys().forEach((key) => {
        if (this.globalHeaders.has(key)) {
          requestHeaders = requestHeaders.set(key, <string>this.globalHeaders.get(key));
        }
      });
    }
    ...
    if (customHeaders) {
      customHeaders.keys().forEach((key) => {
        if (customHeaders.has(key)) {
          requestHeaders = requestHeaders.set(key, <string>customHeaders.get(key));
        }
      });
    }
  ...
  public buildRequestOptions(customOptions: any = {}): object {
